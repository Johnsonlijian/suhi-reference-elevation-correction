# -*- coding: utf-8 -*-
"""Build Figure 3 and print terrain-stratified summary statistics."""
import os
...
for thresh in [0.5, 1.0, 2.0, 3.0]:
    ...
    print(
        f"  |bias|>{thresh}C: all={all_frac:.1f}%, "
        f"terrain={t_frac:.1f}%, flatland={f_frac:.1f}%"
    )
...
named = {
    ...
    (29.0, -2.9): "S. Kivu highlands, DRC",
}
top_ok = d[(d["trust_flag"] == "ok") & (d["abs_bias"] > 5)].sort_values(
...
for _, r in top_ok.head(20).iterrows():
    ...
    if city:
        print(
            f"  {city}: surplus={r['reference_elevation_surplus_m']:.0f}m, "
            f"conv={r['conventional_SUHI_C']:.1f}->"
            f"matched={r['elevation_matched_SUHI_C']:.1f}C, "
            f"bias={r['reference_elevation_bias_C']:.1f}C"
        )

fig = plt.figure(figsize=(12.8, 4.8))
gs = fig.add_gridspec(
    1, 2, width_ratios=[2.5, 1.0], wspace=0.10, left=0.01, right=0.98, top=0.92, bottom=0.06
)
...
cb.set_label("|reference-elevation bias| (deg C)", fontsize=8.5, labelpad=2)
cb.ax.tick_params(labelsize=8.0)
cb.set_ticks([0.5, 1, 2, 3])
cb.ax.set_xticklabels(["0.5", "1", "2", ">=3"])

ax_map.set_title(
    "a   Global distribution of |reference-elevation bias|", loc="left", fontsize=9.5, pad=4
)
ax_map.text(
    0.008,
    0.06,
    "|bias| > 1 deg C in 47.4% of cities  |  median 0.92 deg C (all 11,452)",
    transform=ax_map.transAxes,
    fontsize=8.0,
    color="#222",
    bbox=dict(boxstyle="round,pad=0.22", fc="white", ec="none", alpha=0.80),
    zorder=7,
)
...
cats = [
    ...
    (
        "Moderate terrain\n(200-500 m)",
        (d["abs_surplus"] > 200) & (d["abs_surplus"] <= 500),
        "#F59E0B",
    ),
...
ax_cdf.text(1.08, 0.10, "1 deg C", fontsize=7.5, color="#666")
ax_cdf.set_xlim(0, 5.5)
ax_cdf.set_ylim(0, 1.03)
ax_cdf.set_xlabel("|reference-elevation bias| (deg C)", fontsize=8.5)
ax_cdf.set_ylabel("Cumulative fraction of cities", fontsize=8.5)
ax_cdf.set_title("b   Bias by terrain class", loc="left", fontsize=9.5)
ax_cdf.legend(
    fontsize=8.0,
    loc="lower right",
    frameon=True,
...
for ext in ("png", "pdf", "svg"):
    fig.savefig(os.path.join(OUT, f"Figure3_global_bias_map.{ext}"), dpi=900, bbox_inches="tight")
plt.close()
